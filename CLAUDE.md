...
- **Last session:** 2026-05-28 — Multi-household membership + profile-chip switcher. New `household_members` join table (backfilled from existing profiles → 'owner'); `profiles.household_id` repurposed as "active" pointer. New `get_my_household_id()` validates membership + falls back. `seed_default_categories(uuid)` SQL function. `createHousehold` / `switchHousehold` server actions. Sidebar + mobile drawer chip restructured: avatar + name + household-name subtitle, Popover with switch list + create + settings + sign-out. Settings → Household tab gets a "Households" panel. Worktree `/Users/peteroomen/personal/budget-app-multi-household` on `feature/multi-household`. See `docs/work/2026-05-28-multi-household.md`.
